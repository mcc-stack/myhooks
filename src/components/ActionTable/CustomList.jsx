  ...
  return <React.Fragment>
      ...
      <Overlay align="tr br" visible={visible} target={buttonRef.current}>
        <ReactSortable className={styles.columnSortPanel} handle=".column-handle" list={columns} setList={onChange}>
          {columns.map((item, idx) => <div className="sort-item-container" key={getColumnKey(item)}>
              <div className="sort-item">
                <Checkbox className="sort-checkbox" checked={!item.hidden} onChange={status => onHiddenChange(getColumnKey(item), !status)}>
                  {item.title}
                </Checkbox>

                <Icon className="column-handle" type="ellipsis" size="small" />
              </div>
              {Array.isArray(item.children) && <ReactSortable handle=".column-handle" list={item.children} setList={newState => onColumnChildrenChange(idx, newState)}>
                    {item.children.map(childrenItem => <div key={getColumnKey(childrenItem)} className="sort-item sort-item-children">
                        <Checkbox checked={!childrenItem.hidden} onChange={status => onHiddenChange(getColumnKey(childrenItem), !status)}>
                          {childrenItem.title}
                        </Checkbox>
